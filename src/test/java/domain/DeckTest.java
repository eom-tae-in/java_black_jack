...
public class DeckTest {
    ...
    void check_return_participant_cards() {
        //given
        deck.add(new Card(new Shape("다이아몬드"), new Value("6", 6)));
    ...
    void add_card_to_participant_deck() {
        //when
        deck.add(new Card(new Shape("스페이드"), new Value("K", 10)));

        //then
        assertThat(deck.getCards().size()).isEqualTo(1);
        assertThat(deck.getCards().get(0).getNumber()).isEqualTo(10);
        assertThat(deck.getCards().get(0).getValue()).isEqualTo("K");
        assertThat(deck.getCards().get(0).getValueAndShape()).isEqualTo("K스페이드");
    }
